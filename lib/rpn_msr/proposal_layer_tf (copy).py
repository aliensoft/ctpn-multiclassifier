...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    # (1, H, W, A)
    scores = np.reshape(np.reshape(rpn_cls_prob_reshape, [1, height, width, _num_anchors, 3])[:,:,:,:,1],
                        [1, height, width, _num_anchors])
    scores2 = np.reshape(np.reshape(rpn_cls_prob_reshape, [1, height, width, _num_anchors, 3])[:,:,:,:,2],
                        [1, height, width, _num_anchors])
    tmp = scores
    scores = scores2
    scores2 = tmp
    ...
    if DEBUG:
        ...
        print('score map size: {}'.format(scores2.shape))
    ...
    order2 = scores2.ravel().argsort()[::-1]#score按得分的高低进行排序
    
    if pre_nms_topN > 0:                #保留12000个proposal进去做nms
        order = order[:int(pre_nms_topN/2)]
        order2 = order2[:int(pre_nms_topN/2)]

    #pdb.set_trace()
    order = np.hstack((order,order2))
    ...
    #keep2 = nms(np.hstack((proposals, scores2)), nms_thresh)#进行nms操作，保留2000个proposal
    if post_nms_topN > 0:
        keep = keep[:int(post_nms_topN)]
        #keep2 = keep2[:int(post_nms_topN/2)]
        #keeps = keep  
    ...
    #scoresCombine = np.vstack((scores2, scores))
    bbox_deltas=bbox_deltas[keep,:]
